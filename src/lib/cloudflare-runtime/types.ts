...
export interface R2Object { key: string; etag: string; size: number; httpMetadata?: { contentType?: string }; customMetadata?: Record<string, string> }
export interface R2Bucket { head(key: string): Promise<R2Object | null>; put?(key: string, value: Uint8Array, options?: { httpMetadata?: { contentType?: string }; customMetadata?: Record<string, string>; onlyIf?: { etagDoesNotMatch: "*" }; sha256?: string }): Promise<R2Object | null>; createMultipartUpload(key: string, options?: { httpMetadata?: { contentType?: string } }): Promise<{ uploadId: string }> }
