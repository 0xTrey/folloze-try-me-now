...
const scenarios = [
  ...
  { id: "account-lighthouse-security", archetype: "conservative-enterprise", family: "align", profile: "thin", scenario: "Named-account security evaluation with minimal account evidence.", audience: "Lighthouse security leaders", offer: "access-review ownership", objective: "Set a bounded security evaluation agenda", expectedOutcome: "safe-deterministic-fallback", prohibitedOutput: ["Target Company"] },
...
export const buildFlowBenchmarkFixtures: readonly BuildFlowBenchmarkCase[] = scenarios.map((definition) => {
  ...
  return { id: brief.id, family: definition.family, brief, profile: definition.profile, scenario: definition.scenario, expectedOutcome: definition.expectedOutcome, expectation: { ...(definition.expectedOutcome === "production-page" ? { minimumSections: 4 } : {}), requiredEvidenceRefs: definition.profile === "unsupported-metric" ? [`${brief.id}-capability`] : undefined, prohibitedOutput: definition.prohibitedOutput } };
});
