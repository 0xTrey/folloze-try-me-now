...
const TRANSIENT_ASSET =
  /(?:^|[/_.-])(g2|award|badge|benchmark|report|event|roadshow|webinar|conference|summit|promo(?:tion)?|social[-_ ]?banner)(?:[/_.?-]|$)/i;

/** True for imagery that will not still be true next quarter. */
function isEvergreenAsset(url: string): boolean {
  return !TRANSIENT_ASSET.test(new URL(url, "https://first-party.invalid").pathname);
}
...
function experienceImages(values: string[]): string[] {
  ...
  const evergreen = candidates.filter(isEvergreenAsset);
...
/** The renderer's media slots, in the order the strongest asset should fill. */
const RENDER_ASSET_SLOTS: readonly {
  sectionId: string;
 ...
 *
 * This deliberately does no scoring. Upstream ranking is the decision, and
 * re-reading it out of file names here is exactly the second opinion that let
 * the trace and the DOM disagree. Slot `i` takes image `i`; the rest become
 * designed treatments.
 */
function planFromRankedImages(images: readonly string[]): AssetRenderPlan {
  const ranked = [...new Set(images)];
  return {
    version: "asset-render-plan-v1",
    placements: RENDER_ASSET_SLOTS.slice(0, ranked.length).map((slot, index) => ({
      sectionId: slot.sectionId,
      semanticRole: slot.semanticRole,
      assetRef: ranked[index]!,
      reusable: false,
      required: slot.required === true
    })),
    treatments: RENDER_ASSET_SLOTS.slice(ranked.length).map((slot) => ({
      sectionId: slot.sectionId,
      semanticRole: slot.semanticRole,
...
function imageFigure(
  ...
  const fallbackVariants: Record<string, { label: string; steps: readonly string[] }> = {
    "hero-media": { label: "A clear starting point", steps: ["Context", "Proof", "Action"] },
    "lens-media": { label: "A useful path", steps: ["Question", "Signal", "Move"] },
    "framework-media": { label: "Evidence in motion", steps: ["Fact", "Meaning", "Decision"] },
    "journey-media": { label: "The next useful view", steps: ["See", "Understand", "Apply"] }
  };
  const fallback = fallbackVariants[className] ?? { label: "A focused view", steps: ["Focus", "Evidence", "Next"] };
  return `<figure class="media ${className}${roleClass}"${showFallback ? "" : ' data-no-fallback="true"'}${safeAssetRole}${slotAttribute(slot)}>
    ${showFallback ? `<div class="media-fallback"${allowFallback ? ' data-fallback-kind="experience-blueprint"' : ""} aria-hidden="true">
      <span class="media-fallback-kicker">${escapeHtml(fallback.label)}</span>
      <strong>Context.<br>Proof.<br>Next step.</strong>
      <span class="media-fallback-steps">${fallback.steps.map((step) => `<i>${escapeHtml(step)}</i>`).join("")}</span>
    </div>` : ""}
...
/**
 * Serves the compiled allocation plan to the renderer.
 *
...
function createPlanAssetAllocator(plan: AssetRenderPlan): RenderAssetAllocator {
  ...
  const claimedSubstantiveSources = new Set<string>();
  const sourceIdentity = (assetRef: string) => assetRef.split("?")[0]!.toLowerCase();
  return {
    remaining() {
      return [...placements]
    ...
    claim(slot) {
      ...
      const substantive = !["logo", "decorative"].includes(slot.semanticRole);
      if (substantive && claimedSubstantiveSources.has(sourceIdentity(assetRef))) return undefined;
      claimed.add(key);
      if (substantive) claimedSubstantiveSources.add(sourceIdentity(assetRef));
      return assetRef;
    }
  };
}
...
export function renderExperienceHtml(input: {
  ...
  const assetPlan = input.assetPlan ?? planFromRankedImages(images);
  const assetAllocator = createPlanAssetAllocator(assetPlan);
  ...
  const anchorForRole = (role: WireframeSectionRole, label?: string): string => {
    ...
    if (role === "pathways") {
      return familyProduction ? "application-paths" : "starting-points";
    }
    if (
      role === "agenda" ||
  ...
  const signatureButtons = draft.sections
    .map(
      (section, index) => `<button type="button" data-signature-lens-index="${index}" data-flz-cta-id="signature-${index}">
        <span class="signature-item-copy"><span ${editableBlock(`signature.${index}.copy`, "path-copy")}>${escapeHtml(section.headline)}</span></span>
      </button>`
    )
    .join("");
    ...
    ? `${rolePlanned("proof") ? `<section class="framework-section credibility-anchor" id="credibility-anchor" data-journey-section="credibility-anchor" data-template-primitive="credibility-anchor" ${evidenceAttribute(framework.credibility.evidenceIds)} aria-labelledby="credibility-anchor-heading">
      ...
      ${rolePlanned("context") ? `<section class="framework-section urgency-section" id="why-change-now" data-journey-section="why-change-now" data-template-primitive="urgency" ${evidenceAttribute(framework.urgency.evidenceIds)} aria-labelledby="why-change-now-heading">
        ...
        <div class="argument-sequence">
          <article><span>The change</span><p ${editableBlock("urgency.change", "evidence")}>${escapeHtml(framework.urgency.change)}</p></article>
          <article><span>The consequence</span><p ${editableBlock("urgency.consequence", "body")}>${escapeHtml(framework.urgency.consequence)}</p></article>
          <article><span>The better path</span><p ${editableBlock("urgency.reframe", "body")}>${escapeHtml(framework.urgency.reframe)}</p></article>
        </div>
      </section>` : ""}
      ...
      ${rolePlanned("mechanism") ? `<section class="framework-section mechanism-section" id="outcome-mechanism" data-journey-section="outcome-mechanism" data-template-primitive="mechanism" aria-labelledby="outcome-mechanism-heading">
        <div class="framework-copy">
          ...
          <div class="mechanism-steps">${framework.mechanism.steps
            .map(
              (step, index) => `<article ${evidenceAttribute(step.evidenceIds)}><div><h3 ${editableBlock(`mechanism.${index}.action`, "headline")}>${escapeHtml(step.action)}</h3><p ${editableBlock(`mechanism.${index}.capability`, "body")}>${escapeHtml(step.capability)}</p><strong ${editableBlock(`mechanism.${index}.output`, "outcome")}>${escapeHtml(step.output)}</strong></div></article>`
            )
            .join("")}</div>
        </div>
          ...
          .map((section) => {
            ...
            if (section.role === "decision-support") {
              const criteria = framework.startingPoints.choices
                .map(
                  (choice) =>
                    `<article><h3>${escapeHtml(choice.label)}</h3><p>${escapeHtml(choice.buyerJob)}</p></article>`
                )
                .join("");
            ...
            if (section.role === "pathways") {
              const paths = draft.sections
                .map(
                  (path) =>
                    `<article><h3>${escapeHtml(path.headline)}</h3><p>${escapeHtml(path.body)}</p></article>`
                )
                .join("");
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}[hidden]{display:none!important}html{scroll-behavior:auto;overflow-x:clip;scroll-padding-top:70px}body{margin:0;background:var(--brand-surface);color:var(--brand-ink);font-family:var(--body);line-height:1.5;overflow-x:clip;overscroll-behavior-y:contain;-webkit-font-smoothing:antialiased}button,a{font:inherit;touch-action:manipulation}.hero-copy,.lens-copy,.journey-copy,.close>div,.region-heading{min-width:0}.shell{max-width:1600px;min-height:100dvh;margin:0 auto;background:var(--brand-surface);overflow:clip}.eyebrow{margin:0 0 18px;color:var(--brand-accent);font-size:12px;font-weight:800;letter-spacing:.14em;text-transform:uppercase}.skip-link{position:fixed;left:16px;top:12px;z-index:1000;padding:10px 14px;border-radius:6px;background:var(--brand-ink);color:#fff;text-decoration:none;transform:translateY(-180%);transition:transform .16s ease}.skip-link:focus{transform:translateY(0)}.nav{height:78px;padding:0 clamp(22px,6vw,96px);display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid var(--line);background:var(--brand-surface)}.brand-lockup{display:flex;flex:0 0 auto;align-items:center;gap:12px;min-width:0}.lockup-divider{min-width:38px;min-height:32px;padding:0 9px;display:grid;place-items:center;border-inline:1px solid color-mix(in srgb,var(--muted) 32%,transparent);color:var(--muted);font-size:9px;font-weight:800;letter-spacing:.12em;text-transform:uppercase}.wordmark{display:inline-grid;grid-template-areas:"mark";flex:0 0 auto;align-items:center;max-width:172px;min-height:36px;color:var(--brand-ink);font-weight:800;font-size:19px;line-height:1;white-space:nowrap}.wordmark img,.wordmark-fallback{grid-area:mark}.wordmark img{display:block;width:auto;height:auto;max-width:172px;max-height:32px;object-fit:contain;opacity:0}.wordmark-fallback{max-width:100%;overflow:hidden;text-overflow:ellipsis}.wordmark.has-image img{opacity:1}.wordmark.has-image .wordmark-fallback{visibility:hidden}.target-wordmark{max-width:142px}.target-wordmark img{max-width:142px;max-height:30px}.nav-action{border:0;background:transparent;color:var(--brand-ink);min-height:44px;padding:0;cursor:pointer;font-weight:750}.nav-action:hover,.nav-action:focus-visible{color:var(--brand-accent)}
    ...
    .composition-chapter-journey .hero{min-height:72vh}.composition-chapter-journey .framework-starting-points{background:var(--brand-dark);color:#fff}.composition-chapter-journey .framework-starting-points .eyebrow{color:var(--brand-accent)}.composition-chapter-journey .framework-starting-points .region-intro{color:color-mix(in srgb,#fff 76%,transparent)}.composition-chapter-journey .lens-tabs{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:1px;padding:0;background:color-mix(in srgb,#fff 18%,transparent)}.composition-chapter-journey .lens-tabs button{min-height:92px;padding:18px 20px;border:0;background:var(--brand-dark);color:#fff;text-align:left;white-space:normal}.composition-chapter-journey .lens-tabs button[aria-selected="true"]{background:var(--brand-accent);color:var(--brand-on-accent)}.composition-chapter-journey .lens-panel{padding:clamp(30px,5vw,64px) 0;color:#fff}.composition-chapter-journey .lens-copy>p:not(.eyebrow){color:color-mix(in srgb,#fff 78%,transparent)}
...
<script data-flz-runtime>
  (function(){
    ...
    var allowedEvents={anchor_click:true,cta_click:true,topic_select:true,signature_select:true,question_select:true,section_view:true,section_dwell:true,page_heartbeat:true,experience_view:true,fullscreen_change:true,editable_block_select:true,journey_complete:true};
    var durableEvents={anchor_click:true,cta_click:true,topic_select:true,signature_select:true,question_select:true,section_dwell:true,page_heartbeat:true,experience_view:true,journey_complete:true};
    var stringKeys={area:true,targetId:true,sectionId:true,ctaId:true,lensId:true,position:true,completionKey:true,hrefHost:true,state:true,blockId:true,blockKind:true};
    var textKeys={sectionTitle:96,sectionHeadline:160,lensTitle:96,lensHeadline:160};
    var eventSequence=0;
    var parentOrigin='*';
    ...
    function safeText(value,maxLength){if(typeof value!=='string')return undefined;var clean=value.replace(/[\\u0000-\\u001f\\u007f]+/g,' ').replace(/\\s+/g,' ').trim();return clean?clean.slice(0,maxLength):undefined}
    function cleanPayload(data){
      ...
      Object.keys(textKeys).forEach(function(key){var value=safeText(data[key],textKeys[key]);if(value)clean[key]=value});
    ...
    function lensContext(index){
      var tab=tabs&&tabs[index];
      var panel=tab&&document.getElementById(tab.getAttribute('aria-controls'));
      return {lensTitle:textFor(tab),lensHeadline:textFor(panel&&panel.querySelector('h2,h3'))};
    }
    var tabs=Array.from(document.querySelectorAll('[role="tab"]'));
    var lastSelectedLensContext;
    document.addEventListener('click',function(event){
      var target=event.target;
      var control=target&&typeof target.closest==='function'&&target.closest('.close [data-experience-action]');
      if(!control)return;
      window.flzAnalytic(control.getAttribute('data-action-event')||'cta_click',{area:'close',ctaId:control.getAttribute('data-flz-cta-id')||'close-primary'});
    });
    document.querySelectorAll('[data-scroll-target]').forEach(function(control){
      control.addEventListener('click',function(){
        ...
        var area=this.closest('.close')?'close':this.closest('.journey-nav')?'journey-nav':this.closest('.nav')?'header':this.classList.contains('skip-link')?'skip-link':'hero';
        var actionEvent=this.getAttribute('data-action-event')||'anchor_click';
        window.flzAnalytic(actionEvent,Object.assign({area:area,targetId:target,ctaId:this.getAttribute('data-flz-cta-id')||undefined},sectionContext(target)));
    ...
    document.querySelectorAll('a[target="_blank"]').forEach(function(link){
      link.addEventListener('click',function(event){
        ...
        if(area==='close'&&window.parent&&window.parent!==window)event.preventDefault();
        var hrefHost;
    ...
    function selectLens(tab,focus,announce){
      ...
      if(focus)tab.focus();
      if(announce){lastSelectedLensContext=lensContext(lensIndex);window.flzAnalytic('topic_select',Object.assign({area:'decision-lenses',lensIndex:lensIndex,lensId:'lens-'+lensIndex},lastSelectedLensContext))}
    }
    ...
    document.querySelectorAll('[data-signature-lens-index],[data-resource-lens-index]').forEach(function(control){
      control.addEventListener('click',function(){
        ...
        lastSelectedLensContext=lensContext(index);
        window.flzAnalytic(attribute==='data-resource-lens-index'?'topic_select':'signature_select',Object.assign({area:attribute==='data-resource-lens-index'?'resources':'signature',lensIndex:index,lensId:'lens-'+index,ctaId:this.getAttribute('data-flz-cta-id')||undefined},lastSelectedLensContext));
    ...
    function observeSection(section){
      ...
      if(!viewedSections[sectionId]){viewedSections[sectionId]=true;window.flzAnalytic('section_view',Object.assign({sectionId:sectionId},sectionContext(sectionId)))}
    }
    ...
    var journeyCompleteReported=false;
    var journeyStarted=false;
    function reportJourneyComplete(){
      if(journeyCompleteReported||!journeyStarted)return;
      journeyCompleteReported=true;
      var finalSection=journeySections[journeySections.length-1];
      var finalId=finalSection&&finalSection.getAttribute('data-journey-section');
      window.flzAnalytic('journey_complete',Object.assign({sectionId:finalId,area:'journey-end',position:'final',completionKey:String(experienceSessionId||'preview')+':'+String(finalId||'journey-end')},sectionContext(finalId),lastSelectedLensContext||{}));
    }
    window.addEventListener('scroll',function(){
      journeyStarted=true;
      var root=document.scrollingElement||document.documentElement;
      if(root.scrollTop+window.innerHeight>=root.scrollHeight-8)reportJourneyComplete();
    },{passive:true});
    ...
    // Buyer-facing output omits decorative labels and ordinal markers. The
    // editable attributes remain on substantive copy blocks above.
    .replace(/<p class="eyebrow"[^>]*>[\s\S]*?<\/p>/g, "")
    .replace(/<span class="media-fallback-kicker">[\s\S]*?<\/span>/g, "")
    .replace(/<span class="(?:step-index|role-index|journey-index|signature-index)">[\s\S]*?<\/span>/g, "")
    .replace(/<div class="lens-number"[^>]*>[\s\S]*?<\/div>/g, "");
}
