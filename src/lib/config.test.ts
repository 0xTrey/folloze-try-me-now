...
describe("generation timeout configuration", () => {
  it("defaults to a 52-second quality budget", async () => {
    expect(await generationTimeoutFor("")).toBe(52_000);
  });
  ...
  it("hard-caps overrides below the 60-second public promise", async () => {
    expect(await generationTimeoutFor("90000")).toBe(58_000);
  });

  it("accepts an override inside the bounded window", async () => {
    expect(await generationTimeoutFor("45000")).toBe(45_000);
  });
});
