...
export function experienceDocumentHeaders(nonce?: string) {
  ...
  return {
    ...
    "Content-Security-Policy":
      `default-src 'none'; img-src 'self' data:; font-src 'self' https: data: http://localhost:* http://127.0.0.1:*; style-src 'unsafe-inline' https:; script-src ${scriptSource}; connect-src 'self'; frame-ancestors 'self' https://*.folloze.com http://localhost:*; object-src 'none'; worker-src 'none'; base-uri 'none'; form-action 'none'`
  };
}
