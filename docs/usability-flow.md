...
When an approved image is unavailable, the section uses a content-led layout without a placeholder figure or generic decorative copy. Hero, proof, starting-point, and mechanism layouts reclaim the missing image column. Real approved images keep their allocated section and are not repeated to fill another slot. If an image fails to load, its figure is removed and the section uses the same content-led layout. Wordmark text fallback is unchanged.

Single-digit visual sequences use 1 through 9 without a leading zero. This includes starting-point panels, mechanism steps, urgency sequences, resource cards, role cards, chapter counters, and the review gallery. Builder phase, fact, and artifact lists follow the same rule. Fixed-width times, source identifiers, dates, and business facts are not rewritten.

The app applies the same presentation rules when serving an existing saved experience, including its embedded view. A refresh removes the old placeholder figures and unpads known visual number labels. It does not regenerate the page, change its main copy or approved image allocation, or write a new artifact receipt. Script contents and nonce protection are preserved.
...
The content-led layout and numbering follow-up passed 1,808 tests across 164 files, with the public-page fixture test still opt-in. Browser checks covered all 17 archetypes, the legacy campaign path, and the reported GCP page at 1,363px and 390px widths. No placeholder panels, padded single-digit labels, empty media boxes, or horizontal page overflow remained. The reported starting-point tabs still switched content, and its mechanism list used 1, 2, 3. Approved-image placement and failed-image collapse have regression coverage; no new AI generation or email delivery was needed for these checks.
